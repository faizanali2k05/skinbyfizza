/**
 * Brand decoration only (hero imagery + category tiles).
 * NOT app data — real treatments/appointments come from the API.
 */
...
/** Category tiles shown on the Treatments screen (visual grouping only). */
...
export function tileFor(category?: string | null) {
  return categoryTiles.find((c) => c.key === category)?.image ?? procedurePlaceholder;
}
